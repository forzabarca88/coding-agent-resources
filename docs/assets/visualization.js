(function () {
  ...
  // marks — with direct labels where space allows. Both axes are fitted to
  // the runs currently shown: each starts at a step-rounded data minimum
  // rather than at 0 (see niceAxis). Used by visualization.html only.
  // Everything is derived from the markdown at runtime — nothing is hardcoded.
  ...
  // Axis scale fitted to the data: pick a 1/2/5×10^n tick step sized for
  // about `target` ticks across the data span, then snap both bounds outward
  // to the nearest multiple of that step — the start down to at or below the
  // data min, the end up to at or above the data max. The first and last
  // ticks therefore always land exactly on the axis boundaries, and the
  // scale hugs the shown data on both ends (e.g. data 96..257, 4 target
  // ticks -> step 50, start 50, end 300) instead of always anchoring at 0.
  function niceAxis(min, max, target) {
    // Degenerate guard: no usable data -> a minimal 0..1 axis.
    if (!isFinite(min) || !isFinite(max) || max <= 0) return { min: 0, max: 1, step: 1 };
    // Every shown point shares one value: a zero span can't size a step, so
    // derive it from the value, then pad a step either side so the shared
    // point sits mid-axis rather than on an edge. No clamping: keeping
    // (max - min) an exact multiple of step matters more than a positive
    // start (a negative bound can't occur for the chart's targets).
    if (max - min <= 0) {
      var pad = niceAxis(0, max, target).step;
    ...
    // 1e-9 guards: absorb float error when min/max are already exact
    // multiples of step, so neither bound creeps one step past the data.
    var lo = Math.floor(min / step + 1e-9) * step;
    var hi = Math.ceil(max / step - 1e-9) * step;
    return { min: lo, max: hi, step: step };
  }

  // Ticks from the axis start to its end in even steps. Generated by index
  // (start + i*step) rather than repeated addition, so no float drift
  // accumulates across the run of ticks; formatTick() rounds off the rest.
  function tickValues(min, max, step) {
    var out = [];
    for (var i = 0, n = Math.round((max - min) / step); i <= n; i++) out.push(min + i * step);
    return out;
  }
  ...
  function draw() {
    ...
    // Axis bounds hug the runs currently shown (see niceAxis): each axis
    // starts at the step-rounded data minimum, not 0, and re-fits on every
    // draw as the filters change the visible set.
    var minTokens = Infinity, maxTokens = 0;
    var minTurns = Infinity, maxTurns = 0;
    rows.forEach(function (r) {
      if (r.tokensN < minTokens) minTokens = r.tokensN;
      if (r.tokensN > maxTokens) maxTokens = r.tokensN;
      if (r.turnsN < minTurns) minTurns = r.turnsN;
      if (r.turnsN > maxTurns) maxTurns = r.turnsN;
    });

    var xAxis = niceAxis(minTokens, maxTokens, 6);
    var yAxis = niceAxis(minTurns, maxTurns, 5);
    var xMin = xAxis.min;
    var xMax = xAxis.max;
    ...
    var xs = tickValues(xMin, xMax, stepX);
    var ys = tickValues(yMin, yMax, stepY);

    // Value -> plot coordinate over the [min, max] data window (not 0-based).
    function x(v) { return M.left + ((v - xMin) / (xMax - xMin)) * plotW; }
    function y(v) { return M.top + plotH - ((v - yMin) / (yMax - yMin)) * plotH; }
...
/* ------------------------------------------------------------------ *
  ...
  function drawBreakdown() {
    ...
    // Axis bounds hug the runs currently shown (see niceAxis): the metric
    // axis starts at the step-rounded data minimum, not 0.
    var shown = [], minv = Infinity, maxv = 0;
    order.forEach(function (m) {
      byModel[m].forEach(function (r) {
        shown.push(r);
        if (isFinite(r[mKey])) {
          if (r[mKey] < minv) minv = r[mKey];
          if (r[mKey] > maxv) maxv = r[mKey];
        }
      });
    });
    renderBreakdownLegend(shown);

    var xAxis = niceAxis(minv, maxv, 4);
    var xMin = xAxis.min;
    var xMax = xAxis.max;
    var xStep = xAxis.step;
    var xs = tickValues(xMin, xMax, xStep);
    ...
    function sx(v) { return BRK_LABEL + ((v - xMin) / (xMax - xMin)) * (valRight - BRK_LABEL); }

    // Pinned scale (axis + ticks) — sticks to the top of the rolling rows.
    var axis = [];
    axis.push('<svg class="brk-axis" viewBox="0 0 ' + W + ' ' + BRK_AXH + '" role="img" ' +
      'aria-label="' + esc(mLabel + ' scale — ' + formatTick(xMin) + ' to ' + formatTick(xMax)) + '" pointer-events="none">');
    ...
    order.forEach(function (m, i) {
      ...
      // column (only possible for a value at the axis start).
