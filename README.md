...
- **Evaluation Chart** — `visualization.html`; fetches `data/eval-results.md` at runtime and plots successful runs (context used vs turns), coloured by model, with source, multi-select model filters, and a wildcard search (`*`/`?`, e.g. `*qwen*`, `openrouter/*`) that filters to exactly the runs matching — the pattern is tested against each run's model name and Notes column, and wildcards can span both fields, so `Q4` shows every run whose notes mention Q4, `Q2_K_XL` shows only the runs quantised at Q2_K_XL (not every run of the same model), and `qwen3.8-27b*Q4` shows only the runs of a qwen3.8-27b model quantised at Q4. Selecting a model shows all of its successful in-source runs. The model chips offered for filtering always match the selected source (Provider/Local/All) and the wildcard search is scoped to that source too. A chart-specific **Runs** select — "25 with least context" by default — and a **point size** control (Small / Medium / Large, double the original by default) sit in a caption band attached to the scatter figure itself, not in the shared filter bar, so they are visibly specific to the first chart. Scatter points use the same glyph marks as the range chart below (shape = weight quant, outline = KV quant), tinted by model colour, and hover/focus opens the same tooltip as the breakdown. Below the scatter, a **Runs by Model** (range) chart groups *every* run — passed or failed — by model: one row per model carries range rails for Context Used and Turns. A **Context Used / Turns** toggle shows one rail at a time, and the chosen scale is pinned above the rows as they scroll, so the value is never scrolled out of view. Each mark encodes status (carbon = success, red = failed), weight quantisation by shape, and KV quant by its real value (Q4_0, Q8_0, None…, with None dashed); a stacked bar shows each model's pass/fail split. Marks open the same tooltip as the scatter. Both charts share the filters, but the top-N limit and point size apply to the scatter only. An initially collapsed **Overall findings** section — the same shared `content/overall-findings.md` block as the results page — sits above the sticky filter bar and both charts.
