...
- **Theme** — the site is dark by default (the "carbon sheet"; tokens on `:root` in `assets/styles.css`) with a pale-paper light override on `[data-theme="light"]`. The icon at the top right of the header toggles it: `assets/theme.js` (loaded non-deferred in each page's head, before the stylesheet) applies the saved choice before first paint, `assets/site.js` owns the toggle — persisting to `localStorage` and dispatching `site:themechange` — and the chart page listens for `site:themechange` and re-inks its SVG data colours (inline hex values — they can never track CSS variables — see the `THEMES` map in `assets/visualization.js`).

To change any wording, edit the matching file under `content/`; paths mirror the pages (`content/evaluation-results/specs.md`, `content/visualization/note.md`, …). One exception: `content/overall-findings.md` is a single shared file — the initially collapsed **Overall findings** section on both data pages renders from it, so one edit updates both. To add a page: create the HTML shell with its `data-content` slots, add the content files, add the page to the site nav and footer on every page, and register it in `content/pages.md`.
...
- **Evaluation Result** — `evaluation-results.html`; fetches `data/eval-results.md` at runtime and renders it as HTML (live data, no build step). An initially collapsed **Overall findings** section sits above the table — a native `<details>` block fed by the shared `content/overall-findings.md` (also rendered on the chart page). An interactive filter bar (`assets/results.js`) sticks to the top of the viewport while the tables scroll (mirroring the chart page's filter bar; it unpins on small viewports, where the stacked controls would blanket the screen) and sits above the tables: a free-text search matches any run cell, a status segmented control narrows to (All / Passed / Failed / Over limit, where Passed means exit code 0 with zero failed tests), and a sort control orders runs by the default ranking (most tests passed, then least context used, then fewest turns, following the source file's intended order), or by date, context, duration, or turns. The rendered machine sections are reordered at display time to show the local results before the provider ones (the generated file lists provider first — `data/eval-results.md` is never edited by hand). The status line always reports how many of the total runs are shown, and Clear restores the full table.
- **Evaluation Chart** — `visualization.html`; fetches `data/eval-results.md` at runtime and plots successful runs (context used vs turns), coloured by model, with source, multi-select model filters, and a wildcard search (`*`/`?`, e.g. `*qwen*`, `openrouter/*`) that filters to exactly the runs matching — the pattern is tested against each run's model name and Notes column, and wildcards can span both fields, so `Q4` shows every run whose notes mention Q4, `Q2_K_XL` shows only the runs quantised at Q2_K_XL (not every run of the same model), and `qwen3.8-27b*Q4` shows only the runs of a qwen3.8-27b model quantised at Q4. Selecting a model shows all of its successful in-source runs. The model chips offered for filtering always match the selected source (Provider/Local/All) and the wildcard search is scoped to that source too. A shared **point size** control in the filter bar (Small / Medium / Large; *Medium* is the default) scales the marks of **both** charts at the exact same radius — a *Large* mark is the same size on the scatter and in the range chart (both render at a 920-unit scale), and both legends grow with the setting. A chart-specific **Runs** select — "25 with least context" by default — sits in a caption band attached to the scatter figure itself, not in the shared filter bar, so it is visibly specific to the first chart. Scatter points use the same glyph marks as the range chart below (shape = Model quant, outline = KV quant), tinted by model colour, and hover/focus opens the same tooltip as the breakdown. The scatter carries a legend decoding the same Model quant/KV channels as the breakdown's legend (Model quant shape, KV quant ring) live from the shown runs, swatched at the active size. Below the scatter, a **Runs by Model** (range) chart groups *every* run — passed or failed — by model: one row per model carries range rails for Context Used and Turns. A **Context Used / Turns** toggle shows one rail at a time, and the chosen scale is pinned above the rows as they scroll, so the value is never scrolled out of view. Each mark encodes status (carbon = success, red = failed), weight quantisation by shape, and KV quant by its real value (Q4_0, Q8_0, None…, with None dashed); a stacked bar shows each model's pass/fail split. Marks open the same tooltip as the scatter. Both charts share the filters and the point size, but the top-N limit applies to the scatter only. An initially collapsed **Overall findings** section — the same shared `content/overall-findings.md` block as the results page — sits above the sticky filter bar and both charts.
